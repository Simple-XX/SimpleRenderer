...
{
	device->material[cnt].ambient = am;
	device->material[cnt].diffuse = di;
	device->material[cnt].specular = spe;
	device->material[cnt].shininess = shi;
}
		...
		if (x >= 0 && x < width)
		{
			...
			if (daozp >= zbuffer[x])
			{
				...
				else if (render_state & RENDER_STATE_TEXTURE)
				{
					...
					ff.color.r = (float)texture_r/255.0f;
					ff.color.g = (float)texture_g/255.0f;
					ff.color.b = (float)texture_b/255.0f;
