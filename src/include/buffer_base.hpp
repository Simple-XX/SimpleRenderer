...
template <class T_t> buffer_base_t<T_t>::~buffer_base_t(void) {
  width = 0;
  height = 0;
  return;
}
...
template <class T_t> uint32_t buffer_base_t<T_t>::get_height(void) const {
  return height;
}

template <class T_t> uint32_t buffer_base_t<T_t>::get_width(void) const {
  return width;
}

template <class T_t> void buffer_base_t<T_t>::clear(void) {
  if constexpr (std::is_same_v<T_t, color_t>) {
    std::fill_n(buffer_arr.get(), width * height, color_t::BLACK);
...
template <class T_t> T_t *buffer_base_t<T_t>::to_arr(void) {
  return buffer_arr.get();
}

template <class T_t> const T_t *buffer_base_t<T_t>::to_arr(void) const {
  return buffer_arr.get();
}

template <class T_t> size_t buffer_base_t<T_t>::length(void) const {
