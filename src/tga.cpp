...
TGAColor::TGAColor(const uint8_t _R, const uint8_t _G, const uint8_t _B, const uint8_t _A) : bgra{_B, _G, _R, _A}, bytespp(4) {
    return;
}
...
TGAColor::TGAColor(const uint8_t * _p, const uint8_t _bpp) : bgra{0, 0, 0, 0}, bytespp(_bpp) {
...
TGAImage::TGAImage(const int _w, const int _h, const int _bpp) : data(_w * _h * _bpp, 0), width(_w), height(_h), bytespp(_bpp) {
    return;
}
...
bool TGAImage::write_tga_file(const std::string _filename, const bool _vflip, const bool _rle) const {
    ...
    header.datatypecode = (bytespp == GRAYSCALE ? (_rle ? 11 : 3) : (_rle ? 10 : 2) );
    header.imagedescriptor = _vflip ? 0x00 : 0x20; // top-left or bottom-left origin
...
bool TGAImage::unload_rle_data(std::ofstream &_out) const {
    ...
    while(curpix < npixels) {
        ...
        _out.write(reinterpret_cast<const char *>(data.data() + chunkstart), (raw ? run_length * bytespp : bytespp) );
        if(!_out.good() ) {
            std::cerr << "can't dump the tga file\n";
            return false;
        }
