...
Geometry::Geometry(TGAImage &_image, const string &_filename) : image(_image), filename(_filename) {
...
// 推导过程见
bool Geometry::is_barycentric(const Vectori2 &_vertex1, const Vectori2 &_vertex2, const Vectori2 &_vertex3, const Vectori2 &_p) const {
    // 边向量
...
bool Geometry::is_barycentric(const Vectori3 &_vertex1, const Vectori3 &_vertex2, const Vectori3 &_vertex3, const Vectori3 &_p) const {
    // 边向量
...
Vectori2 Geometry::get_min(const Vectori2 &_vertex1, const Vectori2 &_vertex2, const Vectori2 &_vertex3) const {
...
Vectori2 Geometry::get_max(const Vectori2 &_vertex1, const Vectori2 &_vertex2, const Vectori2 &_vertex3) const {
...
Vectori3 Geometry::get_min(const Vectori3 &_vertex1, const Vectori3 &_vertex2, const Vectori3 &_vertex3) const {
    int x = min(_vertex1.get_vect()[0], min(_vertex2.get_vect()[0], _vertex3.get_vect()[0]) );
    int y = min(_vertex1.get_vect()[1], min(_vertex2.get_vect()[1], _vertex3.get_vect()[1]) );
    int z = min(_vertex1.get_vect()[2], min(_vertex2.get_vect()[2], _vertex3.get_vect()[2]) );
    return Vectori3(x, y, z);
}

Vectori3 Geometry::get_max(const Vectori3 &_vertex1, const Vectori3 &_vertex2, const Vectori3 &_vertex3) const {
    int x = max(_vertex1.get_vect()[0], max(_vertex2.get_vect()[0], _vertex3.get_vect()[0]) );
    int y = max(_vertex1.get_vect()[1], max(_vertex2.get_vect()[1], _vertex3.get_vect()[1]) );
    int z = max(_vertex1.get_vect()[2], max(_vertex2.get_vect()[2], _vertex3.get_vect()[2]) );
    return Vectori3(x, y, z);
}

// 在给定 _image 上按照 _x0, _y0, _x1, _y1 给出的坐标绘制直线，颜色由 _color 指定
// [(_x0, _y0), (_x1, _y1)) 左上角为原点
void Geometry::line(int _x0, int _y0, int _x1, int _y1, const TGAColor &_color) const {
    // 斜率大于 1
    bool steep = false;
...
void Geometry::triangle(const Vectori2 &_vertex1, const Vectori2 &_vertex2, const Vectori2 &_vertex3, const TGAColor &_color) const {
    // 遍历平行四边形，如果在三角形内则填充
    ...
    for(p.set_vect(min.get_vect()[0], 0) ; p.get_vect()[0] <= max.get_vect()[0] ; p.set_vect(p.get_vect()[0] + 1, 0) ) {
        for(p.set_vect(min.get_vect()[1], 1) ; p.get_vect()[1] <= max.get_vect()[1] ; p.set_vect(p.get_vect()[1] + 1, 1) ) {
            if(is_barycentric(_vertex1, _vertex2, _vertex3, p) == true) {
                image.set(p.get_vect()[0], image.get_height() - p.get_vect()[1], _color);
            }
        }
    }
    return;
...
void Geometry::triangle(const Vectori2 * _vertexes, const TGAColor &_color) const {
...
void Geometry::triangle(const Vectori3 &_vertex1, const Vectori3 &_vertex2,
    const Vectori3 &_vertex3, double * _zbuffer, const TGAColor &_color) const {
...
void Geometry::triangle(const Vectori3 * _vertexes, double * _zbuffer, const TGAColor &_color) const {
...
void Geometry::circle(int _x0, int _y0, double _r, double * _zbuffer, const TGAColor &_color) const {
    assert(_r < 0);
    return;
}
