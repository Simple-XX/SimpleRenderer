...
render_t::render_t(const std::shared_ptr<scene_t> &_scene,
    ...
    : scene(_scene), display(_display), input(_input) {
  // 初始化缓冲区
  auto f1 = std::make_shared<framebuffer_t>(WIDTH, HEIGHT);
  auto f2 = std::make_shared<framebuffer_t>(WIDTH, HEIGHT);
  auto f3 = std::make_shared<framebuffer_t>(WIDTH, HEIGHT);
  ...
  while (is_running == true) {
    ...
    for (auto &i : framebuffer) {
      ...
      if (i->displayable == false) {
        ...
        //            vector3f_t(1000, 1000, 1000), vector3f_t(0, 1,
        //            1).normalized(), M_PI_4, vector3f_t(WIDTH / 2, HEIGHT / 2,
        //            0));
