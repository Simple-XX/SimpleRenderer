...
render_t::render_t(const render_t &_render) : scene(_render.scene) { return; }

render_t::~render_t(void) { return; }
...
void display_func(
  ...
  while (1) {
    ...
    for (const auto &i : _framebuffer) {
      ...
      data_cond.wait(lk, [&] { return (i->displayable == true); });
  ...
  while (is_running == true) {
    ...
    for (auto &i : framebuffer) {
      ...
      if (i->displayable == false) {
        ...
        // 右对角线
        i->line(0, config->HEIGHT - 1, config->WIDTH - 1, 0, color_t::WHITE);
        // 左对角线
        i->line(config->WIDTH - 1, config->HEIGHT - 1, 0, 0, color_t::WHITE);
        // 水平平分线
        i->line(config->WIDTH - 1, config->HEIGHT / 2, 0, config->HEIGHT / 2,
                color_t::WHITE);
        // 垂直平分线
        i->line(config->WIDTH / 2, 0, config->WIDTH / 2, config->HEIGHT - 1,
                color_t::WHITE);
