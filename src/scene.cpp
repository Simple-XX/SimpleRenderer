...
scene_t::scene_t(const std::shared_ptr<config_t>& _config) : config(_config) {
    ...
    auto current_camera_shared
      = std::make_shared<camera_base_t>(current_camera);
...
void scene_t::add_model(const model_t& _model) {
    auto aaa
      = model2world_tran(_model, matrix4f_t(), matrix4f_t(), matrix4f_t());
...
bool scene_t::tick(const uint32_t _delta_time) {
    (void)_delta_time;
    /// @todo 更新可见模型
    // 清空队列
...
config_t& scene_t::get_config(void) {
    return *config;
}

const config_t& scene_t::get_config(void) const {
    return *config;
}

camera_base_t& scene_t::get_current_camera(void) {
