...
bool input_t::handle(scene_t &_scene, uint32_t _delta_time) const {
  auto res = true;
  switch (event.type) {
  // 键盘事件
  case SDL_KEYDOWN: {
...
input_t::input_t(void) : event(SDL_Event()) { return; }

input_t::~input_t(void) { return; }

bool input_t::process(scene_t &_scene, uint32_t _delta_time) {
