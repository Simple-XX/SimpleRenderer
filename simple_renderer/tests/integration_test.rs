...
fn setup_shader() -> Shader {
    ...
    shader.set_uniform("viewMatrix", Mat4::look_at_rh(
        Vec3::new(0.0, 0.0, 1.0),
        Vec3::new(0.0, 0.0, 0.0),
        Vec3::Y,
    ));
    shader.set_uniform("projectionMatrix", Mat4::perspective_rh_gl(
        60.0_f32.to_radians(),
    ...
    shader.set_lights(&[
        Light { direction: Vec3::new(1.0, 5.0, 1.0), ..Light::default() },
        Light { direction: Vec3::new(-3.0, -2.0, 2.0), ..Light::default() },
        Light { direction: Vec3::new(2.0, 1.0, -1.0), ..Light::default() },
    ]);
    shader
}
...
fn load_teapot_model() {
    let model = Model::load(TEAPOT_PATH).expect("Failed to load teapot");
    assert!(model.vertices().len() > 100, "teapot should have many vertices");
...
fn render_per_triangle_produces_pixels() {
    ...
    assert!(nonzero > 0, "PerTriangle should produce non-zero pixels, got {}", nonzero);
}

#[test]
fn render_tile_based_produces_pixels() {
    let nonzero = render_with_mode(RenderingMode::TileBased);
    assert!(nonzero > 0, "TileBased should produce non-zero pixels, got {}", nonzero);
}

#[test]
fn render_deferred_produces_pixels() {
    let nonzero = render_with_mode(RenderingMode::Deferred);
    assert!(nonzero > 0, "Deferred should produce non-zero pixels, got {}", nonzero);
}

#[test]
fn render_tile_based_deferred_produces_pixels() {
    let nonzero = render_with_mode(RenderingMode::TileBasedDeferred);
    assert!(nonzero > 0, "TileBasedDeferred should produce non-zero pixels, got {}", nonzero);
}
...
fn all_modes_produce_similar_output() {
    ...
    assert!(
        min_count > 0,
        "All modes should produce output. Counts: {:?}", counts
    );
    // Allow generous variance between modes (different algorithms)
    assert!(
        min_count as f64 / max_count as f64 > 0.3,
        "Modes should produce broadly similar output. Counts: {:?}", counts
    );
}
